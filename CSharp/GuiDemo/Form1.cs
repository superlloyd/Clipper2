...
using Polygon = Path64;
using Polygons = Paths64;
    ...
    public Boolean SaveToFile(string filename, double scale = 1.0, int margin = 10)
    {
      ...
      using (StreamWriter writer = new StreamWriter(filename))
      {
        ...
        foreach (PolyInfo pi in PolyInfoList)
        {
          ...
          (pi.si.pft == FillRule.EvenOdd ? "evenodd" : "nonzero"),
    ...
    using (BinaryReader polyStream = new BinaryReader(_assembly.GetManifestResourceStream("GuiDemo.aust.bin")))
    {
      ...
      for (int i = 0; i < polyCnt; ++i)
      {
        ...
        for (int j = 0; j < vertCnt; ++j)
        {
          ...
          pg.Add(new Point64((int) x, (int) y));
        }
        subjects.Add(pg);
      }
    }
    ...
    using (GraphicsPath path = new GraphicsPath())
    {
      ...
      for (int i = 0; i < count; ++i)
      {
        ...
        foreach (PointF p in path.PathPoints)
          clip.Add(new Point64((int) (p.X * scale), (int) (p.Y * scale)));
        clips.Add(clip);
      }
    }
  }
    ...
    using (StreamReader sr = new StreamReader(filename))
    {
      ...
      for (int i = 0; i < polyCnt; i++)
      {
        ...
        for (int j = 0; j < vertCnt; j++)
        {
          ...
          x = x * scaling + xOffset;
          y = y * scaling + yOffset;
          pg.Add(new Point64((int) Math.Round(x), (int) Math.Round(y)));
        }
      }
    }
    return true;
      ...
      using (GraphicsPath path = new GraphicsPath())
      {
        ...
        using (SolidBrush myBrush = new SolidBrush(Color.FromArgb(127, 0xDD, 0xDD, 0xF0)))
        {
          ...
          if ((clips.Count > 0 || subjects.Count > 0) && !rbNone.Checked)
          {
            ...
            solution.Clear();
#if UsePolyTree
              bool succeeded = c.Execute(GetClipType(), solutionTree, GetFillRule(), GetFillRule());
...
#else
            bool succeeded = c.Execute(GetClipType(), GetFillRule(), solution);
#endif
            if (succeeded)
            {
              ...
              if (nudOffset.Value != 0)
              {
                ...
                co.Execute((double) nudOffset.Value * scale, solution2);
              }
              else
              ...
              using (SolidBrush b = new SolidBrush(Color.Navy))
              {
                ...
                c.Clear();
                c.AddSubject(subjects);
                c.Execute(ClipType.Union, GetFillRule(), solution2);
                foreach (Polygon pg in solution2)
                ...
                c.AddSubject(subjects);
                c.Execute(ClipType.Intersection, GetFillRule(), solution2);
                foreach (Polygon pg in solution2)
